...
//        address payable ownerAddressPayable = _make_payable(ownerAddress); // We need to make this conversion to be able to use transfer() function to transfer ethers
    ...
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        ...
        require(msg.sender == token1OwnerAddress || msg.sender == token2OwnerAddress, "Sender is not the owner of either token.");
        //2. You don't have to check for the price of the token (star)
        //3. Get the owner of the two tokens (ownerOf(_tokenId1), ownerOf(_tokenId2)
        //4. Use _transferFrom function to exchange the tokens.\
        transferFrom(token1OwnerAddress, token2OwnerAddress, _tokenId1);
        transferFrom(token2OwnerAddress, token1OwnerAddress, _tokenId2);
    }
    ...
    function transferStar(address _to1, uint256 _tokenId) public {
        ...
        address ownerAddress = ownerOf(_tokenId);
        require(msg.sender == ownerAddress, "Sender is not the owner of the token.");
